...
import app.cash.backfila.client.misk.dynamodbv2.DynamoDbSegmentingTest.SegmentingBackfill.SegmentingParameters
...
class DynamoDbSegmentingTest {
  ...
  fun `happy case`() {
    ...
    assertThat(run.prepareBackfillResponse.partitions).hasSize(2)
      .extracting<Int> {
        it.backfill_range.start.utf8().split("/").first().last().toString().toInt()
      }
      .contains(0, 4)
